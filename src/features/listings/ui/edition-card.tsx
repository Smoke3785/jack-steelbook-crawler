      ...
      <div className="flex flex-1 flex-col gap-2 p-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
            {[edition.label ? labelName(edition.label) : null, edition.movie_year]
              .filter(Boolean)
        ...
        <div className="mt-auto flex flex-col gap-2">
          <div className="flex flex-wrap gap-1">
            {edition.variant ? <Badge>{edition.variant.replaceAll("-", " ")}</Badge> : null}
            {edition.format ? <Badge>{formatDisplay(edition.format)}</Badge> : null}
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge tone={anyAvailable ? "available" : "sold-out"}>
              {anyAvailable
                ? `${edition.available_count}/${edition.listing_count} in stock`
                : "Sold out"}
            </Badge>
          </div>
